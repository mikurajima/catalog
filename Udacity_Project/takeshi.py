...
def project():
	...
	sql = "SELECT path, COUNT(path) AS count FROM log GROUP BY path ORDER BY count DESC limit 4"
	cur.execute(sql)
	...
	sql = "WITH tb2 AS \
		...
		GROUP BY path\
		ORDER BY count DESC)\
		SELECT articles.author AS author_id, SUM(tb.count) AS page_views\
		FROM articles\
		JOIN tb\
...
# July 29, 2016 ? 2.5% errors
	...
	sql = "WITH tb200 AS\
		...
		FROM log)\
		SELECT date AS date_200, status AS status_200, COUNT(status) AS count_200\
		FROM tb\
		WHERE status = '200 OK'\
		...
		(SELECT date_trunc('day', time) AS date, status\
		FROM log)\
		SELECT date AS date_404, status AS status_404, COUNT(status) AS count_404\
		FROM tb\
		WHERE status = '404 NOT FOUND'\
		GROUP BY date, status\
		ORDER BY date)\
		SELECT tb200.date_200, tb404.count_404, tb200.count_200, (tb404.count_404 * 1.0 / (tb404.count_404 * 1.0 + tb200.count_200 * 1.0)) AS ratio2\
		FROM tb404\
		JOIN tb200\
...
#ALTER TABLE log drop column article_slug;
